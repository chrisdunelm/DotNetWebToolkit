...
namespace Cil2Js {
    public static class Transcoder {
        ...
        public static string ToJs(MethodDefinition method, string jsMethodName, bool verbose = false) {
            var ast = ToAst(method, verbose);
            return Js.Create(method, jsMethodName, ast);
        }

        public static string ToJs(MethodInfo methodInfo, string jsMethodName, bool verbose = false) {
            var method = GetMethod(methodInfo);
            return ToJs(method, jsMethodName, verbose);
        }
